...
try {
    ...
    const training = await request("/api/v1/trainings", {
        ...
        body: JSON.stringify({
            ...
            title: `Treinamento de integraÃ§Ã£o ${suffix}`,
            description: "Treinamento temporÃ¡rio utilizado pela validaÃ§Ã£o da jornada de onboarding.",
            modality: "online",
            workloadMinutes: 30,
            materials: [],
            status: "published",
    ...
    const trainingClass = await request(`/api/v1/trainings/${created.trainingId}/classes`, {
        ...
        body: JSON.stringify({
            departmentId,
            name: `Turma de integraÃ§Ã£o ${suffix}`,
            status: "open",
    ...
    const template = await request("/api/v1/journeys/templates", {
        ...
        body: JSON.stringify({
            ...
            name: `Onboarding temporÃ¡rio ${suffix}`,
            description: "Modelo temporÃ¡rio criado para validar todo o fluxo de jornada e onboarding.",
            kind: "onboarding",
            durationDays: 7,
            status: "published",
            stages: [{
                ...
                tasks: [
                    ...
                    { title: "Alinhamento com lideranÃ§a", taskType: "meeting",
                        responsible: "owner", dueAfterDays: 1,
                        meetingTime: "09:00", meetingDurationMinutes: 30 },
                    { title: "Concluir treinamento de integraÃ§Ã£o", taskType: "training",
